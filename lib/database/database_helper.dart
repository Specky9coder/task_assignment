...
class DatabaseHelper {
  ...
  Future<List<Map<String, dynamic>>> queryAllRows() async {
    Database? db = await instance.database;
    return await db?.query(table,orderBy: "${DatabaseHelper.columnDateTime} DESC") ?? [];
  }

  Future<List<Map<String, dynamic>>> queryRow(String name) async {
    Database? db = await instance.database;
    return await db?.rawQuery(
            "select * from $table where ${DatabaseHelper.columnName}=?",
            [name]) ??
        [];
  }
